...
name = pd.read_csv("Combined_Name_Inferences.csv")


name['Name'] = name['Highest Prob. Score']

name = name.drop(columns=['First Name', 'Last Name', 'Highest Prob. Score'])

face = pd.read_csv("Combined_Face_Inferences.csv")
face = face.drop(columns=['First Name', 'Last Name','White', 'Black', 'Asian', 'Other'])
face['Face'] = face['Highest Prob. Score']
face = face.drop(columns=['Highest Prob. Score'])


truth = pd.read_csv("Combined_Truth.csv")
truth = truth.drop(columns=['First Name', 'Last Name','Image'])


combined_df = pd.concat([face, name, truth], axis=1)
